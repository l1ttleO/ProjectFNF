...
class PlayState extends MusicBeatState
{
	...
	override public function create()
	{
		...
		versionTxt = new FlxText(0, FlxG.height - 24, 0, SONG.song + " - " + CoolUtil.difficultyString(false) + " | ProjectFNF " + MainMenuState.projectFnfVersion, 16);
	...
	override public function update(elapsed:Float)
	{
		...
		if (ClientPrefs.advancedScoreTxt)
			scoreTxt.text = 'Score: ' + songScore + ' | Health: ' + FlxMath.roundDecimal(healthPercentage, 0) + '% | Misses: ' + songMisses + ' | Accuracy: '
				+ FlxMath.roundDecimal(ratingPercent * 100, 2) + '% | Rating: ' + fc + ratingString;
		else
				...
				if (!daNote.mustPress && daNote.wasGoodHit && !daNote.hitByOpponent && !daNote.ignoreNote)
				{
					...
					else if (!daNote.noAnimation)
					{
						...
						if (ClientPrefs.dadNotesDoDamage
							&& (!(health - ClientPrefs.damageFromDadNotes / 10 * 0.02 < 0.001) || ClientPrefs.dadNotesCanKill))
						{
